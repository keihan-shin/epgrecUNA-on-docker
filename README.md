...
途中でチューナー数をお伺いします。epgrec/config.php の `define('TUNER_UNIT1', x);` に設定するチューナー数です。

* 自動生成される docker-compose.yml の中身をエディタで確認します。確認ポイントは以下の 2 点。

1. チューナーデバイスが正しいか否か。devices: 以下の行をご確認ください。デバイス検出は簡易的、ミスがあり得る想定です。 

2. DB に関する情報。初回起動「前」であれば DB のホスト名・ユーザー名・パスワード・データベース名を変更できます。

注） docker-compose.sample (拡張子が「sample」のもの）は雛形、触る必要は無いです。
